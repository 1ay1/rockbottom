...
Verdict Sampler::judge(const Snapshot& s, double dt) const {
    ...
    if (s.cpu.hetero()) {
        const ClusterLoad cl = cluster_load(s.cpu.cores);
        if (cl.misplaced()) {
            std::string ev = "efficiency cores at " + fmt_pct(cl.eff_avg) + " while the " +
                             std::to_string(cl.perf_n) + " performance cores sit at " +
                             fmt_pct(cl.perf_avg);
